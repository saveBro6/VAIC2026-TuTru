...
export function RoomManagementPage() {
  ...
  return <><PageHeader title="Quản lý phòng" description={`${filtered.length} phòng phù hợp · tự cập nhật mỗi 15 giây`}/><div className="card mb-4 grid gap-3 md:grid-cols-[1fr_220px]"><label className="relative"><Search className="absolute left-3 top-3 text-slate-400" size={20}/><input className="form-control pl-10" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Tìm phòng, khoa hoặc bác sĩ..."/></label><select value={status} onChange={(e) => setStatus(e.target.value as Room['status'] | 'ALL')} className="form-control"><option value="ALL">Tất cả trạng thái</option><option value="OPEN">Đang mở</option><option value="PAUSED">Tạm dừng</option><option value="CLOSED">Đã đóng</option></select></div>{query.isLoading ? <LoadingSkeleton/> : filtered.length ? <RoomStatusGrid rooms={filtered} onStatus={(id,next) => mutation.mutate({ id, status: next })}/> : <EmptyState title="Không tìm thấy phòng"/>}</>
}
