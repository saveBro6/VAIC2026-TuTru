  ...
  return <><PageHeader title="Tiếp nhận bệnh nhân" description="Nhập thông tin ban đầu để xác định khu khám phù hợp." />
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
      <form onSubmit={(e) => { e.preventDefault(); const validationError = getValidationError(); if (validationError) { toast.error(validationError); return } routingMutation.mutate() }} className="card space-y-6">
        ...
        <div className="grid gap-5 md:grid-cols-2"><label><span className="field-label">CCCD *</span><input value={form.cccd} onChange={(e) => update('cccd', e.target.value.replace(/\D/g, '').slice(0, 12))} className="form-control px-5" placeholder="000000000001" /></label><label><span className="field-label">Họ và tên *</span><div className="relative"><UserRound className="pointer-events-none absolute left-4 top-3.5 text-slate-400" size={19} /><input value={form.name} onChange={(e) => update('name', e.target.value)} className="form-control pl-14 pr-5" placeholder="Nguyễn Văn An" /></div></label></div>
        <div className="grid gap-5 md:grid-cols-3"><label><span className="field-label">Tuổi *</span><input inputMode="numeric" value={form.age} onChange={(e) => update('age', e.target.value.replace(/\D/g, '').slice(0, 3))} className="form-control px-5" placeholder="35" /></label><label><span className="field-label">Giới tính *</span><select value={form.gender} onChange={(e) => update('gender', e.target.value)} className="form-control px-5"><option value="UNKNOWN">Chưa xác định</option><option value="MALE">Nam</option><option value="FEMALE">Nữ</option><option value="OTHER">Khác</option></select></label><label><span className="field-label">Thai kỳ</span><select value={form.pregnancyStatus} disabled={form.gender !== 'FEMALE'} onChange={(e) => update('pregnancyStatus', e.target.value)} className="form-control px-5 disabled:bg-slate-100 disabled:text-slate-400"><option value="NA">Không áp dụng</option><option value="NO">Không mang thai</option><option value="YES">Đang mang thai</option><option value="UNKNOWN">Chưa rõ</option></select></label></div>
        <label><span className="field-label">Triệu chứng bệnh nhân mô tả *</span><textarea value={form.symptoms} onChange={(e) => update('symptoms', e.target.value)} className="form-control min-h-40 px-5" placeholder="Ví dụ: Đau tai bên phải, chảy dịch tai khoảng hai ngày, không sốt..." /></label>
