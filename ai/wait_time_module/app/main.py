...
from .queue_engine.engine import ELIGIBLE,QueueEngine
from .schemas.api import *
from .services.events import RequoteRequired,apply_event
...
app=FastAPI(title="AI-Native Wait-Time & Queue Engine",version="1.0.0");engine=QueueEngine();predictor=DurationPredictor(os.getenv("MODEL_ARTIFACT_DIR",str(MODULE_ROOT / "artifacts")));estimate_cache={}
...
def event(e:Event):
    if e.actor_id=="unknown":e.actor_id="public";e.actor_type="ANONYMOUS"
...
def estimate_task(t:Task,now:datetime|None=None):
    ...
    result=PatientEstimate(task_id=t.task_id,journey_id=t.journey_id,queue_id=t.queue_id,task_type=t.task_type,clinical_priority=t.clinical_priority,readiness_status=t.readiness_status,estimate=estimate,estimate_status=status,generated_at=now,valid_until=now+timedelta(minutes=2),estimate_version=store.version,reason_codes=reason,eligibility=eligibility,wait_breakdown=wait_breakdown)
    estimate_cache[key]=result;return result

@app.get("/api/v1/tasks/{task_id}/estimate",response_model=PatientEstimate)
def task_estimate(task_id:str):
    task=store.tasks.get(task_id)
    if not task or task.readiness_status in {ReadinessStatus.COMPLETED,ReadinessStatus.CANCELLED,ReadinessStatus.NO_SHOW_HOLD}:raise HTTPException(404,"active task not found")
    return estimate_task(task)

@app.get("/api/v1/queues/{queue_id}/estimates")
def queue(queue_id:str):return [estimate_task(t) for t in engine.order(tasks(queue_id),datetime.now(TZ))]
...
def queue_summary(queue_id:str):
...
def room_options(req:RoomOptionsRequest):
    ...
    now=datetime.now(TZ);return {"request_id":req.request_id,"generated_at":now,"valid_until":now+timedelta(minutes=2),"options":[option_for(q,req,now) for q in req.candidate_room_ids]}

@app.post("/api/v1/estimates/assignment-impact")
def assignment_impact(req:AssignmentImpactRequest):
    if not room_exists(req.room_id):raise HTTPException(422,"INVALID_CANDIDATE_ROOM")
    temp=Task(task_id=f"impact-{uuid4()}",journey_id="dry-run",queue_id=req.room_id,task_type=req.task_type,service_type=req.service_code,clinical_priority=req.clinical_priority,readiness_status=ReadinessStatus.READY,ready_at=req.ready_at,predicted_minutes=req.predicted_minutes,created_seq=store.version+1)
...
def scenario(req:ScenarioRequest):
    ...
    if req.scenario_type=="NO_SHOW":simulated_tasks=[t for t in simulated_tasks if t.task_id!=req.task_id]
        ...
        simulated_tasks.append(Task(task_id="scenario-return",journey_id="dry-run",queue_id=req.queue_id,task_type=TaskType.RETURN_REVIEW,service_type="RESULT_REVIEW",clinical_priority=priority,readiness_status=ReadinessStatus.READY,ready_at=datetime.now(TZ),predicted_minutes=5,created_seq=store.version+1))
