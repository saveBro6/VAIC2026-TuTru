...
from .services.auth import Principal,require_scope
...
def journey(journey_id:str,_=Depends(require_scope("wait.read"))):
    ...
    step_estimates=[estimate_task(t) for t in found]
    return {"journey_id":journey_id,"timestamps":j.model_dump() if j else {},"timestamp_source":{"actual_result_ready_at":"ACTUAL" if j and j.actual_result_ready_at else "UNAVAILABLE","actual_return_arrived_at":"ACTUAL" if j and j.actual_return_arrived_at else "UNAVAILABLE","ready_for_review_at":"DERIVED_FROM_ACTUALS" if j and j.ready_for_review_at else "UNAVAILABLE"},"return_ewt":q.model_dump() if q else None,"steps":[{"task_id":t.task_id,"task_type":t.task_type,"status":t.readiness_status,"queue_id":t.queue_id,"estimate":estimate.estimate.model_dump() if estimate.estimate else None,"estimate_status":estimate.estimate_status,"wait_breakdown":estimate.wait_breakdown,"generated_at":estimate.generated_at,"valid_until":estimate.valid_until,"estimate_version":estimate.estimate_version,"reason_codes":estimate.reason_codes} for t,estimate in zip(found,step_estimates)],"estimate_version":store.version}
